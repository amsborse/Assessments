...
listed in `degraded_locators`. Fallbacks are guarded: a structural (CSS) match whose role differs
from the recorded role is rejected, because the worst failure is clicking the *wrong* element;
checkpoints catch the rest. When nothing resolves at all, replay can opt into one **bounded
repair**: a single model call picks the element on the live screen, the step's own value is used,
irreversible steps are excluded, and the fix is returned as a proposal (`assisted_steps`,
`proposed_targets`) rather than applied to the artifact. Real bugs found and fixed test-first:
stale detached frames after re-login; a handoff that left the step deadline running, so a person
slower than the timeout always came back to a failed checkpoint.
...
**Reuse (built).** Capabilities and profiles are keyed by vendor product and carry no tenant data;
a tenant (`catalog/tenants/`) binds base URL and credentials. Differences are absorbed in layers:
(1) multi-strategy targets — Bayside relabels controls and a column header and the shared artifact
still succeeds, reporting drift; (2) **overlays** — drifting steps are re-derived on that
tenant's own screens into verified targets, saved as a draft overlay pinned to the capability
version, reviewed, and applied only once approved (Bayside then replays with zero drift);
(3) bounded repair or re-discovery for redesigns (Coastal). Drift across many tenants signals a
product version change (new capability version); on one tenant, configuration (an overlay).
...
Left out: desktop/pixel surfaces (seam above), real co-browsing (a polling console with live input
...
one desktop app; (4) operator SSO and a CDP-screencast console; (5) an eval runner for discovery
