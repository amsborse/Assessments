...
class Settings(BaseSettings):
    ...
    # Regexes over URL paths automation may act on (comma-separated), and permitted action types.
    allowed_paths: Annotated[list[str], NoDecode] = Field(default_factory=lambda: [".*"])
    allowed_actions: Annotated[list[str], NoDecode] = Field(
        default_factory=lambda: ["click", "fill", "select", "press", "extract", "navigate"]
    )
    ...
    @field_validator("allowed_paths", "allowed_actions", mode="before")
    @classmethod
    ...
    def _known_actions(cls, value: list[str]) -> list[str]:
        known = {"click", "fill", "select", "press", "extract", "navigate"}
        unknown = set(value) - known
        if unknown:
            raise ValueError(f"ALLOWED_ACTIONS has unknown actions {sorted(unknown)}")
        return value
