...
class PlaywrightWebSurface:
    ...
    async def target_for(self, ref: str, examples: dict[str, str]) -> Target:
        ...
        async def is_it(locator: Locator) -> bool:
            return await _has_ref(locator, ref)

        return await self._verified_target(frame, desc, is_it, examples)

    async def retarget(self, target: Target, params: dict[str, str]) -> Target | None:
        """Re-derive a verified multi-strategy target for the element `target` resolves to now.

        Used when a step only resolved via a fallback strategy (drift): the element is found, so
        describe it afresh on this tenant's UI. Returns None if the target does not resolve.
        """
        _, locator = await self._resolve(target, params)
        frame = self._frame_for(target.scope)
        if locator is None or frame is None:
            return None
        token = uuid.uuid4().hex[:8]
        desc = await locator.evaluate(
            "(e, t) => { e.setAttribute('data-cu-probe', t);"
            " return window.__cu.describeElement(e) }",
            token,
        )

        async def is_it(candidate: Locator) -> bool:
            try:
                return bool(
                    await candidate.evaluate(
                        "(e, t) => e.getAttribute('data-cu-probe') === t", token
                    )
                )
            except PlaywrightError:
                return False

        fresh = await self._verified_target(frame, desc, is_it, params)
        return fresh.model_copy(update={"description": target.description})

    async def _verified_target(
        self,
        frame: Frame,
        desc: dict[str, Any],
        is_it: Callable[[Locator], Awaitable[bool]],
        examples: dict[str, str],
    ) -> Target:
        """Keep only candidate strategies that resolve uniquely to the described element."""
        ...
        if desc.get("href") and not str(desc["href"]).startswith(("javascript:", "#")):
            candidates.append(HrefStrategy(href=desc["href"]))
        if desc.get("text"):
            role = desc.get("role")
            element = "link" if role == "link" else "button" if role == "button" else "any"
        ...
        for strategy in candidates:
            ...
            if count == 1 and locator is not None and await is_it(locator):
                verified.append(_templatize_strategy(strategy, examples))
        if not verified:
            raise TargetNotFound(
                Target(description="element", strategies=candidates), ["none unique"]
            )
    ...
    async def _locate(
        ...
        try:
            match strategy:
                ...
                case LabelStrategy() | TableCellStrategy() | TextStrategy() | HrefStrategy():
                    token = uuid.uuid4().hex[:8]
    ...
    async def _resolve(
        ...
        tried: list[str] = []
        recorded_role = next(
            (s.role for s in target.strategies if isinstance(s, RoleStrategy)), None
        )
        for rank, strategy in enumerate(target.strategies):
            ...
            if (
                ...
                and isinstance(strategy, CssStrategy)
                and recorded_role is not None
                and await locator.evaluate("e => window.__cu.role(e)") != recorded_role
            ):
                # A structural path that now lands on a different *kind* of control is not the
                # recorded element; never act on it.
                tried.append("css=role-mismatch")
                continue
