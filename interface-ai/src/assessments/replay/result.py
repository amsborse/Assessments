...
class ReplayResult(BaseModel):
    ...
    overlay: str | None = Field(
        default=None, description="Overlay applied, e.g. 'bayside (approved)'."
    )
    assisted_steps: list[dict[str, Any]] = Field(
        default_factory=list,
        description="Steps re-found by the opt-in model repair (model, reason, element chosen).",
    )
    proposed_targets: dict[str, Any] = Field(
        default_factory=dict,
        description="Drift repair proposals: step id → freshly verified target (overlay draft).",
    )
