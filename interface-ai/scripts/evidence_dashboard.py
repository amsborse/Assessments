...
CATEGORIES: dict[str, tuple[str, str, str]] = {
    ...
    "assisted": ("Model-assisted", "#0f766e", "A step was re-found by one bounded model repair"),
...
def _tags(run: dict[str, Any]) -> str:
    ...
    for step in run.get("assisted_steps", []):
        tags.append(("model repair:", step))
    if run.get("overlay"):
        tags.append(("overlay", run["overlay"]))
...
def build_dashboard(evidence: Path) -> Path:
    ...
    runs = sorted([*discoveries, *replays], key=lambda r: r["scenario"])
    counts: dict[str, int] = {}
    for r in runs:
    ...
    legend = "".join(
        f'<li><button type="button" data-filter="{key}" style="--c:{colour}" title="{meaning}" '
        f'aria-pressed="false"><span class="sw"></span><b>{label}</b> {counts.get(key, 0)}</button></li>'
    ...
    for d in discoveries:
        ...
        disc_html += f"""
<article class="disc" id="{d["scenario"]}" data-cat="{category(d)}" style="--c:{CATEGORIES[category(d)][1]}">
  ...
  <p class="links">{links}{f' <a href="../{d["artifact"]}">artifact</a>' if d.get("artifact") else ""}</p>
    ...
    for r in replays:
        ...
        cat = category(r)
        item = f"""
<li class="run" id="{r["scenario"]}" data-cat="{cat}" style="--c:{CATEGORIES[cat][1]}">
  <div class="body">
    ...
    cert_html = ""
    if cert:
        pct = round(cert["confidence"] * 100)
        cases = "".join(
            f"<li><span>{html.escape(c['name'])}</span><b>{c['passed']}/{c['runs']}</b></li>"
            for c in cert["cases"]
        )
        verdict = (
            "eligible for unattended use" if cert["eligible_for_approval"] else "not yet eligible"
        )
        cert_html = f"""
<section class="cert" id="certification">
  <div class="gauge" style="--p:{pct}"><span>{pct}%</span></div>
  <div><h3>Confidence {pct}%: {verdict}</h3>
    <p class="meta">Every golden case replayed {cert["runs_per_case"]} times. Pass rate {round(cert["pass_rate"] * 100)}%,
    found by primary locator {round(cert["primary_locator_rate"] * 100)}%, p50 {cert["p50_ms"] / 1000:.1f}s, p95 {cert["p95_ms"] / 1000:.1f}s.
    Approval is refused below 95% or under 5 runs per case.</p>
    <ul class="cases">{cases}</ul>
    <p class="links"><a href="certification.json">certification.json</a> <a href="capability.json">capability.json</a></p></div>
</section>"""
    mcp = summary.get("mcp")
    mcp_html = (
        f"""<p class="meta">An agent called <code>list_capabilities</code>, then
        <code>invoke_capability</code> by name with typed params: <b>{html.escape(str(mcp.get("response_status")))}</b>.
        <a href="mcp-session.json">mcp-session.json</a></p>"""
        if mcp
        else ""
    )
...
.legend button {{ display:flex; align-items:center; gap:7px; color:#e6edf5; background:transparent;
  border:1px solid #2b4163; border-radius:999px; padding:4px 12px; font:inherit; cursor:pointer; }}
.legend button:hover {{ border-color:#5d7596; }}
.legend button[aria-pressed="true"] {{ background:#fff; color:var(--navy); border-color:#fff; }}
.hidden {{ display:none !important; }}
.cert {{ display:flex; gap:24px; align-items:flex-start; flex-wrap:wrap; background:#fff; border:1px solid var(--line);
  border-radius:10px; padding:20px; margin-top:16px; }}
.cert h3 {{ margin:0 0 4px; font-size:16px; }}
.gauge {{ --p:0; width:108px; height:108px; border-radius:50%; flex:none; display:grid; place-items:center;
  background:conic-gradient(#1f7a4d calc(var(--p) * 1%), #e6ebf0 0); }}
.gauge span {{ width:80px; height:80px; border-radius:50%; background:#fff; display:grid; place-items:center;
  font-size:22px; font-weight:700; font-variant-numeric:tabular-nums; }}
.cases {{ list-style:none; padding:0; margin:10px 0 0; display:grid; gap:4px; max-width:420px; font-size:14px; }}
.cases li {{ display:flex; justify-content:space-between; border-bottom:1px solid var(--line); padding:3px 0; }}
...
<main>
  ...
  {"<h2>Beyond the brief: tenants, repair, confidence, agents</h2>" if beyond_rows or cert_html else ""}
  {"<ol class='runs'>" + beyond_rows + "</ol>" if beyond_rows else ""}
  {cert_html}
  {mcp_html}
...
<script>
// Legend doubles as a filter: show only runs of the chosen outcome (click again to clear).
document.querySelectorAll(".legend button").forEach((btn) => btn.addEventListener("click", () => {{
  const on = btn.getAttribute("aria-pressed") !== "true";
  document.querySelectorAll(".legend button").forEach((b) => b.setAttribute("aria-pressed", "false"));
  btn.setAttribute("aria-pressed", String(on));
  document.querySelectorAll("[data-cat]").forEach((el) =>
    el.classList.toggle("hidden", on && el.dataset.cat !== btn.dataset.filter));
}}));
</script>
</body></html>
"""
